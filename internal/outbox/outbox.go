...
func (s *Store) Enqueue(ctx context.Context, topic, eventType, key string, payload any) error {
	if s == nil || s.pool == nil {
...
func (s *Store) ClaimBatch(ctx context.Context, limit int, backoff time.Duration) ([]Row, error) {
	...
	rows, err := s.pool.Query(ctx, `
		WITH due AS (
			SELECT id FROM mes_event_outbox
			WHERE dispatched_at IS NULL AND available_at <= NOW()
			ORDER BY id
			FOR UPDATE SKIP LOCKED
			LIMIT $1
		)
		UPDATE mes_event_outbox o
		SET attempts = o.attempts + 1,
		    available_at = NOW() + $2::interval
		FROM due
		WHERE o.id = due.id
		RETURNING o.id, o.kafka_topic, o.event_type, o.event_key, o.payload, o.created_at,
	...
	for rows.Next() {
		var r Row
		var key *string
		if err := rows.Scan(&r.ID, &r.KafkaTopic, &r.EventType, &key, &r.Payload, &r.CreatedAt,
...
func (p *Publisher) Run(ctx context.Context) {
	...
	for {
		...
		case <-ticker.C:
			...
			if err != nil {
				slog.Warn("mes outbox drain", "err", err)
				continue
			}
			if n >= p.batch {
				_, _ = p.drainOnce(ctx)
...
func (p *Publisher) drainOnce(ctx context.Context) (int, error) {
	...
	for _, r := range rows {
		if err := p.dispatcher.DispatchOutbox(ctx, r); err != nil {
			...
			slog.Warn("mes outbox dispatch failed", "id", r.ID, "type", r.EventType, "err", err)
			continue
		}
