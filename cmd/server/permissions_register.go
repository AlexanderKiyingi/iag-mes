...
import (
	...
	"iag-mes/backend/internal/models"
)
...
func registerPermissionsLoop(ctx context.Context, cfg *config.Config) {
	...
	for {
		...
		err := platformserviceauth.RegisterPermissions(regCtx, saClient, cfg.JWTIssuer, "mes", perms)
		c()
		if err == nil {
			log.Printf("mes: registered %d permissions", len(perms))
			return
		}
		log.Printf("mes: permissions register failed: %v (retry in %s)", err, backoff)
		select {
		case <-ctx.Done():
			return
